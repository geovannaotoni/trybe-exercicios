...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  ...
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // expect(createMenu(example).fetchMenu()).toEqual(example);
    expect(meuRestaurante.fetchMenu()).toEqual(example);
    ...
    // expect(createMenu(example).consumption).toEqual([]);
    ...
    // meuRestaurante.order('coxinha'); não foi necessária essa linha, pois o teste 7 já adicionou a coxinha ao array
    meuRestaurante.order('cerveja');
    expect(meuRestaurante.consumption).toEqual([ 'coxinha', 'cerveja' ]);

    // 10: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.
    meuRestaurante.order('cerveja');
    expect(meuRestaurante.consumption).toEqual([ 'coxinha', 'cerveja' , 'cerveja' ]);
    ...
    // soma dos valores de uma coxinha e duas cervejas mais os 10%
    expect(meuRestaurante.pay()).toBeCloseTo(19.47);
